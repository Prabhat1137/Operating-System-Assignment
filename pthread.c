...
// no of iteration
#define MAXCOUNT 5
// defining time delay for each process in sleep state
#define READER1 50000
#define READER2 100000
#define READER3 400000
#define READER4 800000
#define WRITER1 150000
// creating structure of read-write-lock
// having one mutex variable and two conditional variable
typedef struct
{
    pthread_mutex_t *mut;
    int writers;
...
// creating structure of read-write-arguments
//having one lock of type read-write-lock 
typedef struct
{
    rwl *lock;
    int id;
...
// defining some more required functions
rwargs *newRWargs(rwl *l, int i, long d);
void *reader(void *args);
void *writer(void *args);

// static int type because it can not be initilize anywhere except here
static int data = 1;
...
int main()
{
    ...
    // there are five threads so we have created five arguments
    rwargs *a1, *a2, *a3, *a4, *a5;
    // creating lock variable of read-write-lock type
    rwl *lock;
    //initilizing lock as default
    lock = initlock();
    // initilizing argument and creating thread
    ...
    // waiting for thread process to complete
...
//allocating memory to the argument and initilizing it
rwargs *newRWargs(rwl *l, int i, long d)
{
    rwargs *args;
...
// function return type pointer to void
//argument args of type void
void *reader(void *args)
{
    rwargs *a;
    int d;
...
// function to write the shared data
// function return type pointer to void
//argument args of type void
void *writer(void *args)
{
...
// function to initilized the lock block
// function return type pointer to structure(rwl)
//argument of function of  void type

rwl *initlock(void)
{
    rwl *lock;
    // memory allocation to the lock variable
    lock = (rwl *)malloc(sizeof(rwl));
    if (lock == NULL)
        return (NULL);
    // memory allocation to the mutex variable
    ...
    // memory allocation to the writeok variable
    ...
    // memory allocation to the readok variable
    ...
    // initilizing condition variable
    pthread_cond_init(lock->writeOK, NULL);
    pthread_cond_init(lock->readOK, NULL);
    //initilizing readers,writers,waiting in the locking block 
    lock->readers = 0;
    lock->writers = 0;
    lock->waiting = 0;
...
void readlock(rwl *lock, int d)
{
    ...
    lock->readers++;
    // unlocking the readers
    pthread_mutex_unlock(lock->mut);

    return;
}

// function to block the reader
// function return type  void
//argument lock of type structure of rwl ant int for id of process
void writelock(rwl *lock, int d)
{
    // locking the writers
    pthread_mutex_lock(lock->mut);
    ...
    while (lock->readers || lock->writers)
    {
        ...
        // automatically unlocking & locking mutex->lock and wait for signal or broadcast
...
// function to block the writer
// function return type  void
//argument lock of type structure of rwl
void readunlock(rwl *lock)
{
    //unlocking the reader and wake up one thread each time
    pthread_mutex_lock(lock->mut);
    lock->readers--;
...
// function to unblock the reader
// function return type  void
//argument lock of type structure of rwl
void writeunlock(rwl *lock)
{
    //unlocking the writer and wake up all waiting thread
    pthread_mutex_lock(lock->mut);
    lock->writers--;
...
//deleting all mutex and conditional variable
void deletelock(rwl *lock)
{
    //deleting all mutex
